...
ROOT := github.com/yukitsune/minialert
...
PROJECT_NAME := minialert
DOCKER_COMPOSE_CMD := docker compose --project-name $(PROJECT_NAME) --file ./deployments/docker-compose.yml --env-file ./configs/.env
...
build-container: ## Builds the minialert docker container
	docker build \
		-t minialert \
		-f ./Dockerfile \
		$(DOCKER_BUILD_ARGS) \
